...
export default class DialogRechargeBankchargeProxy extends puremvc.Proxy {
    ...
    api_user_var_coin_recharge_confirm(){
        ...
        if (reg.test(str) && str.length > 1) {
            this.sendNotification(net.HttpType.api_user_var_coin_recharge_confirm, { user_id: core.user_id,order_no: this.pageData.bankTransInfo.order_no,user_recharge_certificate:str});
        }else{
