...
import PageExtensionMediator from "@/_skin004/views/page_extension/mediator/PageExtensionMediator";
import PageExtensionProxy from "@/_skin004/views/page_extension/proxy/PageExtensionProxy";
...
import dialog_performance from "@/_skin001/views/dialog_performance/index";
...
export default class PageExtension extends AbstractView {
    ...
    async savePhoto() {
        ...
        if (this.pageData.qrCode) {
            ...
            let imgData: any;
            if (bg) {
                const myCanvas = new MyCanvas(1000, 1000);
                await myCanvas.drawImage1(bg, 0, 0);
                await myCanvas.drawQrCode(this.pageData.link, 679, 675, 280, 280);
                //推荐人
                const { pretty_user_id, user_id } = this.pageData.promotionData;
                myCanvas.drawText(pretty_user_id || user_id, 845, 649, "#ffffff", 26);
