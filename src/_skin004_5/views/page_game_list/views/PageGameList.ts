...
import PageGameListMediator from "@/_skin004/views/page_game_list/mediator/PageGameListMediator";
...
export default class PageGameList extends AbstractView {
    ...
    public get menudata(): any {
        if (this.listQuery.vendor_type == 1) {
            // @ts-ignore
            return this.hotGame.list;
        }
    ...
    get hotGame() {
        for (const item of this.gameProxy.lobbyIndex) {
            if (item.category == 1) {
                if(this.myProxy.listQuery.vendor_type == 0) {
                    this.myProxy.listQuery.vendor_type = 1;
                }
                return { ...item, vendor_type: 1, vendor_type_name: LangUtil("热门游戏") };
            } else {
                if(this.myProxy.listQuery.vendor_type == 0) {
                    this.myProxy.getFirstMenuIndex();
                }
            }
        }
        return {};
