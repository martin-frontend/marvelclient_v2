...
export default class DialogActivityRankProxy extends puremvc.Proxy {
    ...
    formatLastTimeTxt() {
        ...
        if (time_obj.day) {
            ...
            this.pageData.data.last_time_txt = `${LangUtil("活动剩余")} ${hours}${LangUtil("小时")} ${time_obj.minutes}${LangUtil("分钟")} ${
                time_obj.seconds
            }${LangUtil("秒")}`;
            ...
            this.pageData.data.last_time_txt = `${LangUtil("活动剩余")} ${time_obj.minutes}${LangUtil("分钟")} ${time_obj.seconds}${LangUtil(
                "秒"
            )}`;
        }
    }
