...
export default class BtnUtil extends AbstractView {
    ...
    public get cur_btn_class() : string {
        if (this.btn_type == "nomal" || this.btn_type == 1) //样式一   默认 黄色渐变 黑白一样， 禁用： 白 灰  黑 黑 渐变
        {
            return "btn_base base_border btn_nomal";
        }
        else if (this.btn_type == "two" || this.btn_type == 2) //样式二  渐变边框 透明底  禁用 单色
        {
            return "test_my base_border";
        }
        else if (this.btn_type == "three" || this.btn_type == 3)//样式三  黄色渐变 黑白一样 禁用 单色
        {
            return "btn_base base_border btn_nomal";
        }
        ...
        else if (this.btn_type == "six" || this.btn_type == 6)//样式六  手机版 首页 进入图标 主要区别是 圆角 使用的是 6 
        {
    ...
    public get cur_btn_class_disable() : string {
        ...
        let str="";
        if (this.btn_type == "nomal" || this.btn_type == 1) //样式一   默认 黄色渐变 黑白一样， 禁用： 白 灰  黑 黑
        {
        ...
        else if (this.btn_type == "six" || this.btn_type == 6)//样式六  手机版
        {
            str = "disable_base btn_three_disable";
        }
        else if (this.btn_type == "game_btn" || this.btn_type == 7)//样式七 首页 游戏开始按钮
