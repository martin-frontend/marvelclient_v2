...
export function getRouter(): VueRouter {
    ...
    if (!router) {
        ...
        //如果当前是独立页面，则要处理router
        if (currpath) {
            prePath = "/";
            const currRoute = routes.find((item) => item.path.includes(currpath));
            const noPageRoute = routes.filter((item) => !allRoutes.includes(item.path) && item.path != `${prePath}/`);
            // 新的路由列表
            routes.length = 0;
            if (currRoute) (currRoute.path = "/"), routes.push(currRoute);
            routes.push(...noPageRoute);
        }
