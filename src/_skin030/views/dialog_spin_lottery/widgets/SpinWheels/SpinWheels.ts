...
export default class SpinWheels extends AbstractView {
    ...
    get arrowImg() {
        return require(`@/_skin030/assets/activity_spin/arrow.png`);
    }
    get spinBtnImg() {
        if (this.myProxy.pageData.isSpinning) {
            return require(`@/_skin030/assets/activity_spin/btn/spin_btn_disable.png`);
        }
        return require(`@/_skin030/assets/activity_spin/btn/spin_btn.png`);
    }
    get circleImg() {
        const color = this.myProxy.getColorByLocation(this.myProxy.lotteryAwardData.location);
        return require(`@/_skin030/assets/activity_spin/spinWheels/circles/${color}_circle.png`);
    }
    get roundImg() {
        const color = this.myProxy.getColorByLocation(this.myProxy.lotteryAwardData.location);
        return require(`@/_skin030/assets/activity_spin/spinWheels/roundShapes/${color}Round.png`);
    }
    get colorClass() {
        const color = this.myProxy.getColorByLocation(this.myProxy.lotteryAwardData.location);
