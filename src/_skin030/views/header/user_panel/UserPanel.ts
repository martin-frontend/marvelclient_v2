...
export default class UserPanel extends AbstractView {
    ...
    get menuList() {
        const list = [
            { id: 0, name: LangUtil("个人中心"), icon: "ky_user" },
            { id: 2, name: LangUtil("平台钱包"), icon: "ky_wallet" },
            { id: 3, name: LangUtil("我的投注"), icon: "ky_my_bets" },
            { id: 4, name: LangUtil("消息中心"), icon: "ky_message" },
        ];
        ...
        if (!ModulesHelper.IsShow_HideSafeCenter()) {
            const obj = { id: 1, name: LangUtil("安全中心"), icon: "ky_security" };
            list.splice(1, 0, obj);
        }
        ...
        if (ModulesHelper.IsShow_NoticeBtn()) {
            const obj = { id: 5, name: LangUtil("公告中心"), icon: "ky_support" };
            list.push(obj);
        }
