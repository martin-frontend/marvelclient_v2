...
@Component
export default class NavigationItem extends AbstractView {
    LangUtil = LangUtil;
    core = core;
    ...
    get itemSvgIcon() {
        ...
        //如果为游戏分类的图标
        return this.CategoryIcon[NavigationCommonData.getItemCategory(this.itemData)];
    }
    /**展示的名字 */
    get itemShowName() {
        if (this.itemData.name && this.itemData.name.trim()) return this.itemData.name;

        return NavigationCommonData.getItemCategoryName(this.itemData);
    }
    ...
    onWatchRouter() {
        ...
        if (this.isExpansionPanel && !this.isLanguage) {
            ...
            const id = NavigationCommonData.getItemCategory(this.itemData);
