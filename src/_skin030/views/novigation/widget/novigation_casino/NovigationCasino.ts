...
@Component
export default class NovigationCasino extends AbstractView {
    LangUtil = LangUtil;
    ...
    get novigation_game_menu_history() {
        const list = {
            ...
            7: { name: "近期游戏", id: 7, icon: "ky_recents", action: "opencasino_history" },
            8: { name: "挑战", id: 8, icon: "ky_challenges", action: "" },
        };

        return list;
    }
    /**正常的游戏分类 */
    get novigation_game_type(): core.PlatLobbyIndexVO[] {
        return this.gameProxy.lobbyMenuIndex;
    }
    /**游戏供应商 */
    get novigation_game_vendor() {
        const list = {
    ...
    onGameCategoryClick(item: any) {
        const id = NovigationCommonData.getItemCategory(item);
