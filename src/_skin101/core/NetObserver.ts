    ...
    public handleNotification(notification: puremvc.INotification): void {
        ...
        switch (notification.getName()) {
            ...
            case net.EventType.api_plat_var_lobby_index:
                {
                    let real_index = 1000;
                    for (let i = 0; i < body.class.length; i++) {
                        if (body.class[i].category == 32) {
                            real_index = i;
                        }
                    }
                    if (real_index != 1000) {
                        body.class = JSON.parse(JSON.stringify(body.class));
                        const item = body.class.splice(real_index, 1);
                        body.class.splice(0, 0, item[0]);
                    }
