<template>
    <v-dialog v-model="dialogData.bShow" width="596">
        <v-card class="rounded-lg pb-5" color="colorPanelBg">
            ...
            <v-sheet class="overflow-y-auto mx-4 text-wrapper" color="transparent" height="500">{{ dialogData.data }}</v-sheet>
        </v-card>
    </v-dialog>
</template>
...
@Component
export default class ServiceDialog extends AbstractView {
    private myProxy: ServiceProxy = this.getProxy(ServiceProxy);
